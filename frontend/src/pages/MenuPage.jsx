...
const MenuPage = () => {
    ...
    return (
        <div data-testid="menu-page" className="bg-[#F9F6F0] min-h-screen pt-24">
            {/* Header */}
            <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 pb-10">
                <p className="overline">Il nostro menù</p>
                <h1 className="h-display text-5xl sm:text-7xl mt-5">Ogni piatto, una <span className="italic text-[#C46D46]">storia</span>.</h1>
                <p className="mt-6 text-[#515E4C] max-w-2xl">
                    Sfoglia le categorie, aggiungi al carrello e scegli se ritirare, farti consegnare, o trovarlo pronto al tavolo.
                </p>
            </section>

            {/* Category tabs */}
            <div className="sticky top-16 z-30 bg-[#F9F6F0]/85 backdrop-blur-md border-y border-[#2B4A33]/10">
                        ...
                        {categories.map((c) => (
                            <button
                                ...
                                className={`px-5 py-2.5 rounded-full text-sm font-medium tracking-wide transition-colors whitespace-nowrap ${
                                    active === c.slug
                                        ? "bg-[#2B4A33] text-[#F9F6F0]"
                                        : "bg-transparent text-[#515E4C] hover:bg-[#F1EBE1] hover:text-[#1C231A] border border-[#2B4A33]/15"
                                }`}
                            >
                                {c.name}
                            </button>
                ...
                {activeCategory && (
                    <div className="mb-10 max-w-3xl">
                        ...
                        {activeCategory.description && (
                            <p className="mt-3 text-[#515E4C] leading-relaxed">{activeCategory.description}</p>
                        )}
                    </div>
                )}

                {loading ? (
                    <div className="text-center py-20 text-[#515E4C]">Caricamento...</div>
                ) : visibleItems.length === 0 ? (
                    <div className="text-center py-20 text-[#515E4C] italic font-serif text-2xl">
