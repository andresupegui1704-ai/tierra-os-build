...
function buildScontrinoBytes(o) {
  ...
  const tot = o.items.reduce((s,i)=>s+i.prezzo*i.qty,0);
  return escposBytes([
    ESCPOS.init, ESCPOS.cp858,
    ESCPOS.alignC, ESCPOS.boldOn, ESCPOS.doubleOn,
    "TIERRA\n", ESCPOS.doubleOff,
    ...
    ...o.items.map(i => ppad(`${i.qty}x ${i.nome}`, `EUR ${(i.prezzo*i.qty).toFixed(2)}`)),
    pline("-"),
...
function buildReceiptHTML(ordine, copia) {
  ...
  const items = ordine.items.map(i =>
    `<tr><td>${i.qty}x ${i.nome}</td><td style="text-align:right">€${(i.prezzo*i.qty).toFixed(2)}</td></tr>`
  ).join("");
  const totale = ordine.items.reduce((s,i)=>s+i.prezzo*i.qty,0);
  return `<!DOCTYPE html><html><head>
<meta charset="UTF-8">
<style>
  body{font-family:monospace;font-size:14px;width:300px;margin:0;padding:8px;}
  h2{text-align:center;font-size:16px;margin:4px 0;}
  .center{text-align:center;}
  hr{border:1px dashed #000;margin:6px 0;}
  table{width:100%;}
  .total{font-size:16px;font-weight:bold;}
  .tipo{text-align:center;font-size:18px;font-weight:bold;margin:6px 0;}
</style></head><body>
...
${copia==="cucina"?"":"<div class='center' style='margin-top:8px'>Grazie per la visita! 🌿</div>"}
<br><br><br>
</body></html>`;
}
...
// ORDINI TAVOLI
// ═══════════════════════════════════════════════════════════════════════════
function OrdiniTavoli({ordini, setOrdini, user, menuCatalog}) {
  ...
  const aggiungiPiatto = () => {
    ...
    if(!piatto) return;
    setFormNuovo(f=>({...f, items:[...f.items,{...piatto,qty:parseInt(qtyPiatto)||1}]}));
    setSelectedPiatto(""); setQtyPiatto("1");
  };

  const inviaOrdine = async () => {
    if(!formNuovo.tavolo||formNuovo.items.length===0) return;
    const totale = formNuovo.items.reduce((s,i)=>s+i.prezzo*i.qty,0);
    const ordine = {
    ...
    tierraOrders.create({
      ...
      items: formNuovo.items.map(i=>({
        ...
        quantity: i.qty, unit_price: i.prezzo, customizations: [],
      })),
    ...
    try {
      const tok = await larkToken();
      const righeOrdine = ordine.items.map(i=>"• "+i.qty+"x "+i.nome+" — €"+(i.prezzo*i.qty).toFixed(2)).join("\n");
      await larkSend(tok, {
        schema:"2.0",
            ...
            <div style={{background:C.surface2,borderRadius:8,padding:10,marginBottom:10}}>
              {formNuovo.items.map((item,i)=>(
                <div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"4px 0",borderBottom:`1px solid ${C.border}`}}>
                  <span style={{fontSize:13}}>{item.qty}x {item.nome}</span>
                  <div style={{display:"flex",gap:8,alignItems:"center"}}>
                    <span style={{fontFamily:"'DM Mono',monospace",fontSize:12,color:C.gold}}>€{(item.prezzo*item.qty).toFixed(2)}</span>
              ...
              <div style={{display:"flex",justifyContent:"space-between",padding:"6px 0 0",fontWeight:700}}>
                <span>Totale</span>
                <span style={{fontFamily:"'DM Mono',monospace",color:C.gold}}>€{formNuovo.items.reduce((s,i)=>s+i.prezzo*i.qty,0).toFixed(2)}</span>
              </div>
            </div>
          )}
  ...
  return (
    <>
      ...
      <div style={{minHeight:"100vh",background:C.bg,paddingBottom:76}}>
        ...
        {view==="tavoli"    && <OrdiniTavoli ordini={ordiniTavoli} setOrdini={setOrdiniTavoli} user={user} menuCatalog={menuCatalog}/>}
